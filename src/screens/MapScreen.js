...
export const MapScreen = ({}) => {
  ...
  const searchCurrentLocation = async (locationData) => {
    try {
      const response = await fetch(
        `${process.env.API_GOUV_END_POINT}/reverse/?lon=${locationData.longitude}&lat=${locationData.latitude}&limit=1`
      );
      const data = await response.json();
      if (data.features) {
      ...
      >
        {stepList.length >= 2 && (
          <>
            ...
            />
            {stepListExtremity?.start && (
              <Marker
                coordinate={{
                  latitude: stepListExtremity.start.latitude,
                  longitude: stepListExtremity.start.longitude,
                }}
              >
            ...
            {stepListExtremity?.end && (
              <Marker
                coordinate={{
                  latitude: stepListExtremity.end.latitude,
                  longitude: stepListExtremity.end.longitude,
                }}
              >
                <View style={styles.extremityMarker} />
              </Marker>
            ...
            <Marker
              coordinate={{ latitude: 47.825279, longitude: -1.841667 }}
            ...
            <Marker
              coordinate={{ latitude: 47.616667, longitude: -1.45 }}
              style={styles.userPin}
            >
              <Image
                source={require("../../assets/images/camion_user.png")}
